...
func TestIstioConfigDelete_InvalidType(t *testing.T) {
	...
	r.HandleFunc("/api/namespaces/{namespace}/istio/{object_type}/{object}", func(w http.ResponseWriter, r *http.Request) {
		...
		IstioConfigDelete(w, r.WithContext(ctx))
	}).Methods(http.MethodDelete)

	ts := httptest.NewServer(r)
	defer ts.Close()

	req, err := http.NewRequest(http.MethodDelete, ts.URL+"/api/namespaces/ns/istio/invalidtype/foo", nil)
	if err != nil {
		t.Fatal(err)
	}
